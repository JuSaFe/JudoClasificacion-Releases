...
Bibliotecas que JudoClasificacion incorpora y bajo qué licencia. Todas son permisivas y compatibles con la
...
(la única del otro proyecto con licencia dual): el tablón no genera PDF.

---

## 1. Paquetes NuGet

### Aplicación de escritorio (`JudoClasificacion.csproj`)
...
### Cliente de la API (`JudoClasificacion.Client`) y contratos (`JudoClasificacion.Shared`)
...
JudoClasificacion **no** enlaza con ningún ensamblado de JudoAdministración: los dos repositorios son
independientes. Lo que comparten son los contratos de la API —las clases de `JudoClasificacion.Shared`, que
